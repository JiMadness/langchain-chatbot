  ...
  implements DocumentLoader
{
  ...
  private readonly extractor: (text: string) => string;
  private readonly maxDepth: number;
  private readonly timeout: number;
  private readonly preventOutside: boolean;
  ...
  private getChildLinks(html: string, baseUrl: string): Array<string> {
    ...
    return Array.from(
      new Set(absolutePaths.map((path) => path.replace(/#.*$/g, ''))),
    );
  }
  ...
  private async getChildUrlsRecursive(
    ...
    const isExcluded = this.excludeDirs.some((exDir) =>
      inputUrl.startsWith(exDir),
    );
